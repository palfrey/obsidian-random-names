...
const specialCases: { [key: string] : string } = {"xMens": "X-men", "sciFiGuns": "Sci-fi guns", "twentiethCenturyEnglishs": "20th Century English", "englishs": "English"};
...
function toPretty(str: string): string {
	...
	if (str.includes("_")) {
		...
		return [str.slice(0, num+1), str.slice(num+1)].map(toPretty).join(" ");
	} else {
...
export default class RandomNames extends Plugin {
	async onload() {
		_.keys(choices).sort().forEach(first => {
			_.keys(choices[first]).sort().forEach(second => {
				this.addCommand({
